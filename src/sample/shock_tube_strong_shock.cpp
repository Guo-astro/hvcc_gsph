...
namespace sph
{
    ...
    void load_shock_tube_strong_shock(std::shared_ptr<Simulation> sim,
                                      std::shared_ptr<SPHParameters> param)
    {
#if DIM != 1
        THROW_ERROR("DIM != 1 for shock_tube_strong_shock");
#endif

        // Define the domain boundaries for the fluid region.
        const real L = 4;     // fluid region from -L to L
        const real R = L + 4; // fluid region from -L to L

        // Define resolutions for fluid particles on the left and right.
        const real dx_fluid_left = 0.02;  // left state (high pressure)
        const real dx_fluid_right = 0.02; // right state (low pressure)
        ...
        for (real x = -L + 0.5 * dx_fluid_left; x < 0.0; x += dx_fluid_left)
        {
            ...
            p_i.pres = 10000.0;             // left state pressure (strong shock)
        ...
        for (real x = 0.0 + 0.5 * dx_fluid_right; x < R; x += dx_fluid_right)
        {
            ...
            p_i.pres = 0.0001; // right state pressure
        ...
        const int num_wall_layers = 32; // e.g., three layers of ghost particles per side
        ...
        for (int i = 0; i < num_wall_layers; ++i)
        {
            ...
            wall.vel[0] = 0.0;   // fixed wall
            wall.dens = 1.0;     // match left state density
            wall.pres = 10000.0; // match left state pressure
        ...
        for (int i = 0; i < num_wall_layers; ++i)
        {
            ...
            wall.pres = 0.0001; // match right state pressure
    ...
    REGISTER_SAMPLE("shock_tube_strong_shock", load_shock_tube_strong_shock);
