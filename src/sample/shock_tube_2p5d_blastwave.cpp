...
namespace sph
{
    ...
    void load_shock_tube_2p5d_blastwave(std::shared_ptr<Simulation> sim,
                                        std::shared_ptr<SPHParameters> param)
    {
#if DIM != 3
        THROW_ERROR("DIM != 3 for 2p5d shock_tube");
#endif

        // Retrieve gamma from simulation parameters
        const double gamma = param->physics.gamma;
        WRITE_LOG << " gamma: " << gamma;
    ...
    REGISTER_SAMPLE("shock_tube_2p5d_blastwave", load_shock_tube_2p5d_blastwave);
