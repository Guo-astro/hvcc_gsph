...
#pragma omp parallel for
        for (int i = 0; i < num; ++i)
        {
            ...
            {
                continue; // Skip point masses in fluid interactions
            }
    ...
    real FluidForce::artificial_conductivity(const SPHParticle &p_i, const SPHParticle &p_j, const vec_t &r_ij, const vec_t &dw_ij)
    {
        ...
        const real v_sig = m_use_gravity ? std::abs(inner_product(p_i.vel - p_j.vel, r_ij) / std::abs(r_ij)) : std::sqrt(2.0 * std::abs(p_i.pres - p_j.pres) / (p_i.dens + p_j.dens));
