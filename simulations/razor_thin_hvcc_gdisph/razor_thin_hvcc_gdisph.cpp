...
/**
 * @brief Razor-Thin HVCC Flyby Simulation (GDISPH version for checkpoint testing)
 * 
 * Production-quality simulation of a high-velocity cloud complex (HVCC) 
 * encountering an intermediate-mass black hole (IMBH) using GDISPH.
 * 
 * This simulation:
 * - Uses GDISPH algorithm for better shock capturing
 ...
 * - Configured for checkpoint/resume testing
 */
class RazorThinHVCCGDISPHPlugin : public SimulationPlugin {
public:
    std::string get_name() const override {
        return "razor_thin_hvcc_gdisph";
    }
    
    std::string get_description() const override {
        return "Razor-thin HVCC flyby with GDISPH - Checkpoint/Resume test configuration";
    }
    
    std::string get_version() const override {
        return "2.0.0";  // Version 2.0 - plugin-based architecture with checkpoint support
    }
    ...
    void initialize(std::shared_ptr<Simulation> sim,
        ...
        std::cout << "Initializing Razor-thin HVCC flyby simulation (GDISPH)...\n";
        ...
        std::cout << "  SPH method: GDISPH (Godunov-DISPH)\n";
        ...
        // The solver will call the checkpoint system which loads disk data
        ...
        param->type = SPHType::GDISPH;  // Use GDISPH
        param->time.end = 30.0;
    ...
    std::vector<std::string> get_source_files() const override {
        return {"razor_thin_hvcc_gdisph.cpp"};
    }
};
...
// Export the plugin
DEFINE_SIMULATION_PLUGIN(sph::RazorThinHVCCGDISPHPlugin)
