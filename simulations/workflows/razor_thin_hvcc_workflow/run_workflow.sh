#!/bin/bash
# Automated 2-Step Razor-Thin HVCC Workflow
# Step 1: Generate relaxed disk using Lane-Emden profile + density relaxation
# Step 2: Run IMBH flyby simulation with relaxed disk as initial conditions
...
echo -e "${BLUE}============================================${NC}"
echo -e "${BLUE}  Razor-Thin HVCC Workflow (2-Step)${NC}"
echo -e "${BLUE}============================================${NC}"
echo
echo -e "${GREEN}Step 1:${NC} Disk Relaxation (Lane-Emden + Hydrostatic Equilibrium)"
echo -e "${GREEN}Step 2:${NC} IMBH Flyby Simulation"
echo
...
RUN_STEP1=true
RUN_STEP2=true
SPH_METHOD="disph"  # Default: DISPH
SKIP_BUILD=false
...
if [ "$RUN_STEP2" = true ]; then
    ...
    if [ "$SPH_METHOD" = "gdisph" ]; then
        PLUGIN_PATH="../../../razor_thin_hvcc_gdisph/build/librazor_thin_hvcc_gdisph_plugin.dylib"
        CONFIG_FILE="config_gdisph.json"
        echo -e "${BLUE}Using GDISPH method (better shock capturing)${NC}"
    else
        PLUGIN_PATH="../../../razor_thin_hvcc/build/librazor_thin_hvcc_plugin.dylib"
        CONFIG_FILE="config_disph.json"
        echo -e "${BLUE}Using DISPH method${NC}"
    fi
    ...
    if [ "$SPH_METHOD" = "gdisph" ]; then
        OUTPUT_DIR=$(ls -td output_gdisph/run_* 2>/dev/null | head -1)
    else
        OUTPUT_DIR=$(ls -td output/run_* 2>/dev/null | head -1)
    fi
